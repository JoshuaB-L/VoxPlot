#!/usr/bin/env python3
"""
Enhanced Data analysis module for VoxPlot with fixes for:
1. Equal layer division by voxel count
2. Improved crown layer analysis
...
from utils import (
    ...
    create_equal_layer_masks, validate_layer_division
)


class ForestStructureAnalyzer:
    """Analyzes forest structure from voxel-based density data with improved layer division."""
    ...
    def analyze_crown_layers(self, df: pd.DataFrame, density_type: str,
        ...
        """
        Analyze crown layers with equal voxel count division and calculate density metrics for each layer.
        
        This enhanced version ensures each layer contains approximately the same number of
        valid voxel points above the minimum density threshold, providing more balanced
        analysis compared to simple height-based division.
        ...
        """
        self.logger.debug(f"Analyzing crown layers for {density_type} with equal voxel count division")
        ...
        # Calculate layer boundaries using equal voxel count method
        ...
        # Create layer masks with equal voxel counts
        z_min = crown_df['z'].min()
        z_max = crown_df['z'].max()
        
        layer_masks = create_equal_layer_masks(crown_df, layer_boundaries)
        
        # Validate layer division for quality assurance
        layer_stats = validate_layer_division(crown_df, layer_masks, density_type, min_density)
        self.logger.debug(f"Layer division statistics: {layer_stats}")
        ...
        # Analyze individual layers with equal voxel counts
        ...
        for i, (layer_name, mask) in enumerate(zip(layer_names, layer_masks)):
            if len(mask) > 0 and isinstance(mask, np.ndarray):
                ...
                # Log layer information for verification
                valid_voxels = len(layer_df[layer_df[density_type] > min_density])
                self.logger.debug(f"{layer_name.capitalize()} layer: {valid_voxels} valid voxels")
            else:
                layer_metrics[layer_name] = self._empty_layer_metrics()
                layer_rasters[layer_name] = None
        ...
        return {
            ...
            'layer_statistics': layer_stats  # Added for validation
        }
    ...
    def _analyze_single_layer(self, layer_df: pd.DataFrame, density_type: str, 
        ...
        # Create 2D raster array with improved missing data handling
        ...
        # Only create raster data if we have valid data
        raster_data = None
        if len(raster_array) > 0 and np.any(raster_array > 0):
            raster_data = {
                'array': raster_array,
                'x_coords': x_coords,
                'y_coords': y_coords,
    ...
    def _create_vertical_slice(self, crown_df: pd.DataFrame, density_type: str) -> Optional[Dict[str, Any]]:
        ...
        # Filter out zero density values for cleaner visualization
        slice_data_copy = slice_data_copy[slice_data_copy[density_type] > 0]
        
        if len(slice_data_copy) == 0:
            return None
        ...
        if len(z_coords) == 0 or len(x_coords) == 0:
            return None
    ...
    def analyze_vertical_profile(self, df: pd.DataFrame, density_type: str,
        ...
        """
        Analyze vertical density profile with improved handling of sparse data.
    ...
    def analyze_spatial_distribution(self, df: pd.DataFrame, density_type: str,
        ...
        """
        Analyze spatial distribution of density values with improved data handling.
        ...
        # Create spatial raster with improved handling
        raster_array, x_coords, y_coords = create_2d_raster(
...
class ComparativeAnalyzer:
    """Performs comparative analysis between multiple datasets with enhanced layer division."""
    ...
    def compare_crown_metrics(self, datasets: List[pd.DataFrame], density_type: str,
        ...
        """
        Compare crown layer metrics between multiple datasets using equal voxel count division.
        ...
        """
        self.logger.info(f"Comparing crown metrics for {len(datasets)} datasets with equal layer division")
        ...
        for df in datasets:
            ...
            # Perform crown layer analysis with equal voxel count division
            ...
            # Log layer validation statistics
            if 'layer_statistics' in layer_analysis:
                self.logger.debug(f"Layer balance for {model_name}: {layer_analysis['layer_statistics']}")
