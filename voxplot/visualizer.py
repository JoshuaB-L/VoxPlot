#!/usr/bin/env python3
"""
Enhanced Visualization module with fixes for:
1. Font availability and performance issues
2. SVG text rendering problems  
3. PDF text editability
4. Optimized matplotlib configuration
"""
...
class FontManager:
    """Manages font selection and validation for optimal performance."""
    
    def __init__(self):
        self.logger = logging.getLogger(__name__)
        self._available_fonts = None
        self._validated_fonts = {}
        self._setup_font_cache()
    
    def _setup_font_cache(self):
        """Setup font cache and identify available fonts."""
        try:
            # Get list of available font families
            available_fonts = set([f.name for f in fm.fontManager.ttflist])
            self._available_fonts = available_fonts
            self.logger.info(f"Font cache initialized with {len(available_fonts)} available fonts")
        except Exception as e:
            self.logger.warning(f"Font cache setup failed: {e}")
            self._available_fonts = set()
    
    def get_best_font(self, preferred_fonts: List[str], fallback: str = 'DejaVu Sans') -> str:
        """
        Get the best available font from a list of preferences.
        
        Args:
            preferred_fonts: List of preferred font names
            fallback: Fallback font if none of the preferred fonts are available
        
        Returns:
            Name of the best available font
        """
        # Check cache first
        cache_key = tuple(preferred_fonts)
        if cache_key in self._validated_fonts:
            return self._validated_fonts[cache_key]
        
        # Find the first available font
        for font in preferred_fonts:
            if self._is_font_available(font):
                self._validated_fonts[cache_key] = font
                self.logger.debug(f"Selected font: {font}")
                return font
        
        # Check if fallback is available
        if self._is_font_available(fallback):
            self._validated_fonts[cache_key] = fallback
            self.logger.debug(f"Using fallback font: {fallback}")
            return fallback
        
        # Use matplotlib generic family as last resort
        generic_font = 'sans-serif'
        self._validated_fonts[cache_key] = generic_font
        self.logger.debug(f"Using generic font family: {generic_font}")
        return generic_font
    ...
    def _is_font_available(self, font_name: str) -> bool:
        ...
        for available_font in self._available_fonts:
    ...
    def get_system_fonts(self) -> List[str]:
        """Get list of available system fonts for debugging."""
        return sorted(list(self._available_fonts or []))


class PublicationStyleManager:
    """Manages publication-quality styling with optimized font handling."""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Typography settings with system-aware font selection
        typography = viz_config.get('typography', {})
        
        # Define font preferences with better fallbacks
        preferred_fonts = [
            typography.get('title_font', 'sans-serif'),
            typography.get('body_font', 'sans-serif')
        ]
        
        # Get the best available font
        self.primary_font = self.font_manager.get_best_font(
            preferred_fonts + ['DejaVu Sans', 'Liberation Sans', 'Arial', 'Helvetica'],
            fallback='sans-serif'
        )
        
        self.logger.info(f"Selected primary font: {self.primary_font}")
        ...
        self.subtitle_weight = typography.get('subtitle_weight', 'normal')  # Changed from 'semibold'
        ...
        # Initialize matplotlib settings with optimized configuration
        self._setup_matplotlib_style()
    
    def _setup_matplotlib_style(self):
        """Configure matplotlib for publication quality with optimized text rendering."""
        # Font configuration using validated font
        plt.rcParams['font.family'] = self.primary_font
        plt.rcParams['font.size'] = self.label_fontsize
        
        # Text rendering optimization
        plt.rcParams['text.usetex'] = False  # Disable LaTeX for better compatibility
        plt.rcParams['mathtext.default'] = 'regular'  # Use regular fonts for math text
        plt.rcParams['font.serif'] = [self.primary_font]
        plt.rcParams['font.sans-serif'] = [self.primary_font]
        plt.rcParams['font.monospace'] = [self.primary_font]
        
        # SVG and PDF text rendering optimization
        plt.rcParams['svg.fonttype'] = 'none'  # Keep text as text in SVG
        plt.rcParams['pdf.fonttype'] = 42  # TrueType fonts for editable PDF text
        plt.rcParams['ps.fonttype'] = 42   # TrueType fonts for PS output
        ...
        self.logger.info("Matplotlib style configured for optimized text rendering")
    ...
    def get_font_properties(self, font_type: str) -> Dict[str, Any]:
        """Get font properties for different text types using validated font."""
        properties = {
...
class ForestStructureVisualizer:
    """Creates publication-quality visualizations with optimized text rendering."""
    
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.logger = logging.getLogger(__name__)
        
        # Initialize style manager with font optimization
        ...
        self.logger.info("Publication-quality visualizer initialized with optimized text rendering")
    
    def create_crown_layer_comparison(self, comparison_results: Dict[str, Any], 
                                    density_type: str) -> plt.Figure:
        """Create enhanced crown layer comparison with optimized text rendering."""
        ...
        # Improved grid spacing with more space for titles and between plots
        ...
        gs = gridspec.GridSpec(
            ...
            hspace=0.45,  # Increased vertical spacing
            wspace=0.25,  # Increased horizontal spacing  
            top=0.88,     # More space at top for main title
            bottom=0.10,  # Space for labels
            left=0.10,    # Space for row labels
            right=0.88    # Space for colorbar
        )
        
        # Set main title with optimized font properties
        ...
        fig.suptitle(f'Crown Layer Comparison - {density_label}', 
                    ...
                    y=0.95)  # Higher position for main title
        ...
        for row_idx, row_label, layer_key in row_info:
            # Add row label with optimized font properties
            row_props = self.style_manager.get_font_properties('subtitle')
            fig.text(0.02, 0.78 - row_idx * 0.15, row_label,  # Adjusted vertical positioning
                    ha='center', va='center', 
            ...
            for col_idx in range(n_models):
                ...
                # Add model name title for top row only with optimized text properties
                if row_idx == 0:
                    title_props = self.style_manager.get_font_properties('subtitle')
                    ax.text(0.5, 1.15, model_name, ha='center', va='bottom',
                ...
                # Set axis labels with optimized font properties
    ...
    def _plot_crown_layer_raster_enhanced(self, ax, analysis, layer_key, density_type, vmin, vmax):
        """Enhanced crown layer raster plotting with proper white background for missing data."""
        ...
        if raster_data and raster_data['array'].size > 0 and np.any(raster_data['array'] > 0):
            # Create masked array to handle zeros/missing data properly
            raster_array = raster_data['array'].copy()
            
            # Mask areas with no data (zeros) to show as white
            masked_array = np.ma.masked_where(raster_array <= 0, raster_array)
            
            # Plot raster with enhanced colormap, masked areas will be white
            im = ax.imshow(masked_array, origin='lower', 
                         cmap=self.style_manager.density_colormap,
                         vmin=vmin, vmax=vmax, extent=raster_data['extent'])
            
            # Add metrics text with optimized font properties
            ...
            # Enhanced text box styling with optimized font
            ...
            ax.text(0.5, 1.08, metrics_text, ha='center', va='bottom',
                   transform=ax.transAxes, 
    ...
    def _plot_vertical_slice_enhanced(self, ax, analysis, density_type):
        """Enhanced vertical slice plotting with proper white background."""
        ax.set_facecolor('white')  # Ensure white background
        
        zx_slice = analysis.get('zx_slice')
        if zx_slice and zx_slice['array'].size > 0 and np.any(zx_slice['array'] > 0):
            # Dynamic color scale for vertical slices
            slice_array = zx_slice['array'].copy()
            slice_data = slice_array[slice_array > 0]
            
            if len(slice_data) > 0:
                slice_vmax = np.percentile(slice_data, 95)
                slice_vmax = max(slice_vmax, 0.1)
                
                # Create masked array for proper white background
                masked_slice = np.ma.masked_where(slice_array <= 0, slice_array)
                
                im = ax.imshow(masked_slice, origin='lower', 
                             cmap=self.style_manager.density_colormap,
    ...
    def _plot_no_data_message(self, ax):
        """Plot consistent 'no data' message with guaranteed white background."""
        # Ensure white background
        ax.set_facecolor('white')
        ax.clear()  # Clear any existing content
        ax.set_facecolor('white')  # Set again after clear
        ...
        # Remove ticks and ensure clean white background
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    def _add_enhanced_colorbar(self, fig, vmin, vmax, density_label):
        """Add enhanced colorbar with optimized text properties."""
        colorbar_pos = [0.91, 0.15, 0.015, 0.7]  # Adjusted position
        ...
        # Style colorbar with optimized font properties
        label_props = self.style_manager.get_font_properties('label')
    ...
    def save_figure_with_optimized_text(self, fig: plt.Figure, output_dir: Path, 
                                      base_filename: str, analysis_type: str = "general") -> List[Path]:
        """Save figure with optimized text rendering for different formats."""
        ...
        for fmt in formats:
            try:
                ...
                # Get format-specific DPI and settings
                if fmt == 'png':
                    ...
                    save_kwargs = {
                        'metadata': {'Creator': 'VoxPlot', 'Producer': 'matplotlib'}
                    }
                elif fmt == 'svg':
                    dpi = export_config.get('svg_dpi', 600)
                    save_kwargs = {
                        'metadata': {'Creator': 'VoxPlot'}
                    }
                else:
                    dpi = self.dpi
                    save_kwargs = {}
                
                # Save with optimized settings for text rendering
                fig.savefig(
                    filepath,
                    dpi=dpi,
                ...
                self.logger.info(f"Saved {fmt.upper()} figure with optimized text: {filepath}")
    ...
    def create_enhanced_3d_visualization(self, datasets: List[pd.DataFrame], 
        ...
        """Create publication-quality 3D voxel visualization with improved positioning."""
        if not datasets:
        ...
        fig.suptitle(f'3D Voxel Structure - {density_label}', 
                    ...
                    fontfamily=title_props['family'],
                    color=title_props['color'],
                    y=0.96)  # Higher position to avoid subplot overlap
        ...
        for i, df in enumerate(datasets):
            ...
            # Create 3D subplot with improved spacing
            ax = fig.add_subplot(grid_rows, grid_cols, i + 1, projection='3d')
            ax.set_facecolor(self.style_manager.plot_background)
            
            # Enhanced 3D cube plotting with depth shading
            self._plot_enhanced_3d_cubes_with_depth(ax, crown_df, density_type, cmap, norm)
            
            # Style the 3D plot with better title positioning
            title_props = self.style_manager.get_font_properties('subtitle')
            ax.set_title(model_name, 
                        fontsize=title_props['size'] - 2,  # Slightly smaller for subplots
                        fontweight=title_props['weight'],
                        fontfamily=title_props['family'],
                        color=title_props['color'],
                        pad=15)  # Reduced padding to avoid overlap
        ...
        # Add colorbar with improved positioning to avoid overlap
        if n_models > 0:
            # Position colorbar to avoid overlap with plots
            if grid_cols == 1:
                cbar_pos = [0.94, 0.15, 0.02, 0.7]
            else:
                cbar_pos = [0.02, 0.15, 0.015, 0.7]  # Move to left side for better positioning
        ...
        plt.tight_layout(rect=[0.05, 0, 0.95, 0.92])  # Adjusted to accommodate colorbar and title
        return fig
    
    def _plot_enhanced_3d_cubes_with_depth(self, ax, crown_df, density_type, cmap, norm):
        """Plot enhanced 3D cubes with depth shading and ambient occlusion effect."""
        voxel_size = 0.25  # Could be made configurable
        cube_size_factor = self.three_d_layout.get('cube_size_factor', 0.9)
        cube_alpha = self.three_d_layout.get('cube_alpha', 0.8)  # Slightly higher for better visibility
        edge_width = self.three_d_layout.get('cube_edge_width', 0.1)  # Thinner edges
        edge_color = self.three_d_layout.get('cube_edge_color', '#2C3E50')
        
        cube_size = voxel_size * cube_size_factor
        half_size = cube_size / 2
        
        # Sort by z-coordinate for better depth rendering
        crown_df_sorted = crown_df.sort_values('z')
        
        # Plot cubes for each voxel with depth shading
        for idx, (_, row) in enumerate(crown_df_sorted.iterrows()):
            ...
            # Cube faces with different shading for depth perception
            faces = [
                [vertices[0], vertices[1], vertices[2], vertices[3]],  # bottom (darker)
                [vertices[4], vertices[5], vertices[6], vertices[7]],  # top (lighter)
                [vertices[0], vertices[1], vertices[5], vertices[4]],  # front 
                [vertices[2], vertices[3], vertices[7], vertices[6]],  # back (darker)
                [vertices[0], vertices[3], vertices[7], vertices[4]],  # left (medium)
                [vertices[1], vertices[2], vertices[6], vertices[5]]   # right (medium)
            ]
            
            base_color = cmap(norm(density))
            
            # Apply different shading to faces for depth perception
            face_shadings = [0.7, 1.0, 0.9, 0.6, 0.8, 0.8]  # Different brightness for each face
            
            for face, shading in zip(faces, face_shadings):
                # Adjust color brightness for depth effect
                shaded_color = [min(1.0, c * shading) for c in base_color[:3]] + [cube_alpha]
                
                cube_collection = Poly3DCollection([face], facecolors=shaded_color, 
                                                 alpha=cube_alpha,
                                                 linewidths=edge_width, 
                                                 edgecolors=edge_color)
                ax.add_collection3d(cube_collection)
    
    # Include other visualization methods with minimal changes for space...
...
class EnhancedResultsManager:
    """Enhanced results manager with optimized text export."""
    ...
    def __init__(self, output_dir: str, config: Dict[str, Any]):
        ...
        self.logger.info(f"Enhanced results manager initialized with optimized text export")
    ...
    def save_enhanced_comparison_results(self, comparison_results: Dict[str, Any], 
        ...
        """Save enhanced comparison results with optimized text rendering."""
        ...
        # Crown layer comparison with optimized text
        ...
        visualizer.save_figure_with_optimized_text(crown_fig, self.figures_dir, base_filename, "crown_layers")
        plt.close(crown_fig)
