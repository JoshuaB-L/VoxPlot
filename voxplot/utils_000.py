#!/usr/bin/env python3
"""
Utility functions for VoxPlot analysis.

This module provides common utility functions used across the VoxPlot application,
including mathematical operations, data validation, and helper functions.
"""
...
def calculate_crown_layer_bounds(df: pd.DataFrame, height_column: str = 'z', 
    ...
    """
    Calculate crown layer boundaries based on height distribution.
...
def create_2d_raster(df: pd.DataFrame, voxel_size: float, 
    ...
    """
    Create 2D raster array from point data.
    ...
    # Aggregate to grid
    grid_df = aggregate_to_grid(df, voxel_size, density_column, aggregation_method, x_column, y_column)
